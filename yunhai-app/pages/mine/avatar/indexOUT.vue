				...
				ctx.draw(true, () => {
					...
					uni.canvasToTempFilePath({
						...
						success: function(res) {
							console.log(res)
							uni.hideLoading()
							ucenterApi.uploadAvatarBase(res.tempFilePath).then((resp) => {
								store.commit('SET_AVATAR', resp.url)
							});
						}
					})
				})
				_this.ucenter.avatar = store.getters.avatar
				ucenterApi.updateUcenterInfo(_this.ucenter, null).then(response => {
